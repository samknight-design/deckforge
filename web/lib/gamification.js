...
// recordEvent is best-effort and never throws into the caller's hot path; it
// returns a summary of what was earned so the UI can show a notification.
import { ACHIEVEMENTS, levelFromXp, levelRewards, periodKeyFor } from './tiers';
...
// base XP per event (publish handled via challenge/achievement to avoid farming)
const EVENT_XP = {
  scan: 2,
  insight: 5,
  like_given: 1,
...
// Progress any active DB challenges matching this event. Returns earned XP +
// the names of challenges completed right now.
async function progressChallenges(svc, userId, type, n) {
  const { data: challenges } = await svc
    .from('challenges')
    .select('key, name, period, target, xp')
    .eq('active', true)
    .eq('metric', type);
  if (!challenges || !challenges.length) return { xp: 0, completed: [] };

  let xp = 0;
  const completed = [];
  for (const c of challenges) {
    const period_key = periodKeyFor(c.period);
    ...
    if (!claimed && progress >= c.target) { claimed = true; xp += c.xp; completed.push({ name: c.name, xp: c.xp }); }

    if (row) {
      await svc.from('user_tasks').update({ progress, claimed, updated_at: new Date().toISOString() }).eq('id', row.id);
    } else {
      await svc.from('user_tasks').insert({ user_id: userId, task_key: c.key, period_key, progress, claimed });
    }
  }
  return { xp, completed };
}
...
async function evalAchievements(svc, userId, metrics) {
  ...
  for (const a of ACHIEVEMENTS) {
    ...
    if ((metrics[a.metric] || 0) >= a.threshold) {
      ...
      xp += a.xp;
      unlocked.push({ name: a.name, icon: a.icon, xp: a.xp });
    }
  }
...
// Record a gamified event. Returns a rewards summary:
// { xp, leveledTo|null, achievements:[{name,icon,xp}], challenges:[{name,xp}],
//   scanCredits, insightCredits }
export async function recordEvent(svc, userId, type, n = 1) {
  ...
  try {
    ...
    const ch = await progressChallenges(svc, userId, type, n);
    const ac = await evalAchievements(svc, userId, { ...p, ...updates });
    xpGain += ch.xp + ac.xp;
    ...
    const beforeL = levelFromXp(beforeXp);
    const afterL = levelFromXp(afterXp);
    let scanReward = 0;
    let insightReward = 0;
    for (let L = beforeL + 1; L <= afterL; L++) {
    ...
    return {
      xp: xpGain,
      leveledTo: afterL > beforeL ? afterL : null,
      achievements: ac.unlocked,
      challenges: ch.completed,
      scanCredits: scanReward,
      insightCredits: insightReward,
    };
  } catch (e) {
