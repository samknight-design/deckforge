...
import { findCorners, warpToCanvas } from '@/lib/cardWarp';

// Visual card matching — the free, zero-cost scan path (ManaBox-style).
//
// Pipeline per frame:
//   1) Find the card's 4 corners via Hough-line detection, then perspective-
//      warp into a canonical upright rectangle (kills tilt and centring drift,
//      no WASM, no external scripts). Falls back to a gradient bounding box
//      when Hough doesn't find 4 confident edges.
//   2) Hash via in-JS box-average dHash (engine-independent — agrees with the
//      Node build script bit-for-bit).
...
// Per-pixel luma precomputed once for the whole buffer — sub-rect dHashes then
// just average over the luma plane (no per-pixel multiplies inside the variant
// loop). Major saving when 7 variants share a buffer.
let lumaCache = null;
let lumaCacheKey = null;
function ensureLuma(data, bufW, bufH) {
  const key = `${bufW}×${bufH}×${data.length}`;
  if (lumaCacheKey === key && lumaCache?.length === bufW * bufH) return lumaCache;
  if (!lumaCache || lumaCache.length !== bufW * bufH) {
    lumaCache = new Uint16Array(bufW * bufH);
  }
  // Rec.601 approximation via integer math: (77·R + 150·G + 29·B) >> 8
  // matches 0.299/0.587/0.114 to within 0.4% — well below dHash thresholds.
  for (let p = 0, j = 0; j < lumaCache.length; p += 4, j++) {
    lumaCache[j] = (77 * data[p] + 150 * data[p + 1] + 29 * data[p + 2]) >> 8;
  }
  lumaCacheKey = key;
  return lumaCache;
}

// dHash from a sub-rectangle of the cached luma plane. Indexes into a 1D
// precomputed luma buffer rather than re-reading RGBA + recomputing grayscale.
function dhashFromSubRect(luma, bufW, x0, y0, w, h, size = HASH_SIZE) {
  const gw = size + 1, gh = size;
  const grid = new Float64Array(gw * gh);
  for (let gy = 0; gy < gh; gy++) {
...
function frameHashes(video, opts = {}) {
  ...
  // PATH A — Hough corner detection + perspective warp.
  // When we get 4 confident corners, warp the card into a clean upright
  // rectangle matching the reference framing exactly. Then dHash needs only
  // ONE pass (alignment is exact), and at full reference accuracy.
  const corners = findCorners(video, vf);
  if (corners) {
    const warped = warpToCanvas(video, corners, 244, 340);
    if (warped) {
      const ctx = warped.getContext('2d', { willReadFrequently: true });
      const data = ctx.getImageData(0, 0, warped.width, warped.height).data;
      return {
        hashes: [dhashFromImageData(data, warped.width, warped.height)],
        card: vf,
        detected: true,
        path: 'warp',
      };
  ...
  // PATH B — Fallback: gradient bounding-box + variant hashing.
  // No reliable corners → use the previous-round approach (gradient detect +
  // 7 alignment-tolerant variants at native resolution). When the gradient
  // detector also gives up (det.detected === false), it returns the
  // viewfinder rect as-is — meaning we hash exactly what the user framed,
  // which is the simple/predictable behaviour the user expects when fitting
  // a card inside the on-screen rectangle.
  ...
  const W = Math.round(bufSrcW), H = Math.round(bufSrcH);
  const p = regionPixels(video, bufSrcX, bufSrcY, bufSrcW, bufSrcH); // native res
  const luma = ensureLuma(p.data, W, H);
  ...
  for (const v of VARIANTS) {
    ...
    hashes.push(dhashFromSubRect(luma, W, x0, y0, cw, ch));
  }
  return { hashes, card: base, detected: det.detected, path: 'gradient' };
}
...
export function matchCard(video, opts = {}) {
  ...
  return {
    ...
    detected: fh.detected, variants: fh.hashes.length, path: fh.path,
  };
}

// Multi-frame consensus with aggressive early-exit. With the 7-variant
// query, single-frame matches are already near-perfect on clean cards — the
// voting is insurance for borderline scans. So: 2 frames max, but frame 1
// short-circuits the rest if it's clearly correct (most cases).
//
// Returns confident=false when the win isn't decisive — that's the seam
// where Claude Smart Scan takes over.
export async function matchCardVoted(video, opts = {}, frames = 2) {
  ...
  for (let f = 0; f < frames; f++) {
    ...
    if (m) {
      ...
      // Early exit on first-frame win: distance ≤ 18% of bits AND gap ≥ 10%.
      // Empirically these never flipped to a different card on the second
      // frame, so the second frame is pure latency. Loosened from 15%/12% so
      // more matches qualify for the fast path.
      const total = m.totalBits;
      if (f === 0 && m.distance <= total * 0.18 && (m.runnerUp - m.distance) >= total * 0.10) {
        break;
      }
    }
    if (f < frames - 1) await sleep(10);
  }
