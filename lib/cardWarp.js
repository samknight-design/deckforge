...
const DOWN_W = 200;       // ROI downscale width for edge detection
const HOUGH_THETA_STEPS = 90;  // 0..π in 90 buckets (2° resolution)
const EDGE_THRESH_FRAC = 0.5;  // keep edges in top 50% of magnitude
...
export function findCorners(video, vf) {
  ...
  // Vertical: combine the two wraparound bands.
  const vert = [
    ...topPeaks(hough, 0, Math.floor(ts * 0.18), 4, Math.floor(DW * 0.3)),
    ...topPeaks(hough, Math.floor(ts * 0.82), ts, 4, Math.floor(DW * 0.3)),
  ].sort((a, b) => b.votes - a.votes).slice(0, 6);
  const horiz = topPeaks(hough, Math.floor(ts * 0.38), Math.floor(ts * 0.62), 4, Math.floor(DH * 0.3));
  ...
  if (wAvg < DW * 0.4 || hAvg < DH * 0.4) return null;
  const aspect = wAvg / hAvg;
  if (aspect < 0.5 || aspect < 0 || aspect > 0.95) return null;
