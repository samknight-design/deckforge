...
// Exact printing lookup: set code + collector number → the specific card the
// user is holding (correct art / full-art / set / price), including basic lands.
export async function fetchCardBySetAndNumber(setCode, collectorNumber) {
  ...
  const url = `${SCRYFALL_BASE}/cards/${encodeURIComponent(set)}/${encodeURIComponent(num)}`;
  const res = await fetch(url, { headers: SCRYFALL_HEADERS, next: { revalidate: 3600 } });
  if (!res.ok) return null;
  const data = await res.json();
  if (data.object === 'error') return null;
...
// Name + set: gets the right printing when the collector number is unreadable
// but the set code is legible.
export async function fetchCardByNameAndSet(name, setCode) {
  if (!name) return null;
  const params = new URLSearchParams({ fuzzy: name });
  if (setCode) params.set('set', String(setCode).toLowerCase().trim());
...
// Fetch every printing of a given card name, ordered by release date (newest
// first). Used by the "Change variant" sheet so the user can swap a scanned
// card to the specific printing they own.
export async function fetchPrints(cardName) {
  if (!cardName) return [];
  // !"Card Name" = exact-name match. unique=prints returns one row per printing.
