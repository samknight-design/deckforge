...
function detectCardRect(video, opts = {}) {
  ...
  const aspect = bw / bh;
  const plausible = bw > DW * 0.45 && bh > DH * 0.45 && aspect > 0.55 && aspect < 0.85;
...
// Multi-frame consensus: hash several frames and pick the card that wins most
// often (tie-break by lowest distance). Smooths frame-to-frame instability.
export async function matchCardVoted(video, opts = {}, frames = 5) {
  if (!db) return null;
  const votes = new Map();
  ...
  for (let f = 0; f < frames; f++) {
    ...
    if (m) {
      ...
      const e = votes.get(m.id) || { count: 0, best: 1e9, m };
      e.count++; e.best = Math.min(e.best, m.distance);
      if (m.distance < e.best) e.m = m;
      votes.set(m.id, e);
    }
    if (f < frames - 1) await sleep(45);
  }
  let win = null;
  for (const e of votes.values()) {
    if (!win || e.count > win.count || (e.count === win.count && e.best < win.best)) win = e;
  }
  if (!win) return null;
  return { ...win.m, distance: win.best, votes: win.count, frames, detected: detectedAny };
}
