...
// dHash from a 17×16 RGBA pixel buffer — bit layout identical to the build script.
function dhashFromPixels(d, size = HASH_SIZE) {
...
function drawForHash(source, sx, sy, sw, sh) {
  const w = HASH_SIZE + 1, h = HASH_SIZE;
  const canvas = getWorkCanvas(w, h);
  const ctx = canvas.getContext('2d', { willReadFrequently: true });
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = 'high'; // match @napi-rs/canvas in the build script
...
function frameHash(video, opts = {}) {
  ...
  const d = drawForHash(video, card.x, card.y, card.w, card.h);
  return { bytes: dhashFromPixels(d), card };
}

// Pipeline self-test: hash a full image element/bitmap (the whole card, no crop),
// the way the build script does. Used to verify reference⇄camera pipeline parity.
export function hashImageElement(source) {
  const sw = source.naturalWidth || source.width;
  const sh = source.naturalHeight || source.height;
  if (!sw || !sh) return null;
  return dhashFromPixels(drawForHash(source, 0, 0, sw, sh));
}
