...
import { detectAndWarp, warmCv, cvStatus } from '@/lib/cardDetect';
...
// dHash of the framed card. Tries opencv warp; on any failure falls through to
// the gradient detector + plain crop so the scan ALWAYS produces a hash.
async function frameHash(video, opts = {}) {
  ...
  return {
    bytes: dhashFromImageData(data, c.width, c.height),
    canvas: c,
...
export async function matchCard(video, opts = {}) {
  ...
  return {
    ...
    detected: fh.detected, engine: fh.engine,
  };
}
